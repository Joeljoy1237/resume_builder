...
const STYLES: Record<TemplateId, TemplateStyle> = {
  ...
  academic: {
    ...
    accentRule: "thin",
    headingStyle: "caps-thin",
    summaryLabel: "Research Interests",
    bodySize: "text-[10.5pt]",
    order: [
      "summary",
      "education",
...
function Heading({ title, style }: { title: string; style: TemplateStyle["headingStyle"] }) {
  if (style === "boxed")
    return (
      <h2 className="text-[10pt] font-bold uppercase tracking-wider border border-black px-2 py-0.5 mb-2 mt-5">
        {title}
      </h2>
    );
  if (style === "caps-thin")
    return (
      <h2 className="text-[9.5pt] font-semibold uppercase tracking-[0.2em] text-neutral-700 mt-5 mb-2">
        {title}
      </h2>
    );
  if (style === "left-bar")
    return (
      <h2 className="text-[10pt] font-bold uppercase tracking-wider border-l-2 border-black pl-2 mt-5 mb-2">
        {title}
      </h2>
    );
  return (
    <h2 className="text-[10pt] font-bold uppercase tracking-wider border-b border-black/70 pb-1 mb-2 mt-5">
      {title}
    </h2>
  );
}
...
export function ResumePreview({ data }: { data: ResumeData }) {
          ...
          <section key={key}>
            ...
            {r.skills.technical.length > 0 && (
              <p className="text-[10pt]">
                <span className="font-semibold">Technical:</span> {r.skills.technical.join(", ")}
              </p>
            )}
            {r.skills.soft.length > 0 && (
              <p className="text-[10pt] mt-0.5">
                <span className="font-semibold">Soft:</span> {r.skills.soft.join(", ")}
              </p>
            )}
          </section>
        ) : null;
            ...
            {r.projects.map((p) => (
              <div key={p.id} className="mb-2">
                <div className="text-[11pt]">
                  ...
                  {p.tech && <span className="text-neutral-700"> · {p.tech}</span>}
                </div>
                ...
                <div className="flex justify-between text-[11pt]">
                  <strong>
                    ...
                    {e.school ? (
                      <>
                        {" "}
                        — <span className="font-normal">{e.school}</span>
                      </>
                    ) : null}
                  </strong>
                  <span className="text-[9.5pt] text-neutral-700 whitespace-nowrap">
  ...
  const rule =
    s.accentRule === "double"
      ? "border-t-[3px] border-double border-black mt-3"
      : s.accentRule === "thin"
        ? "border-t border-neutral-400 mt-3"
        : s.accentRule === "none"
          ? "mt-3"
  ...
  return (
    <div
      className="resume-canvas mx-auto shadow-sm"
      style={{ width: "8.5in", minHeight: "11in", padding: s.padding }}
    >
